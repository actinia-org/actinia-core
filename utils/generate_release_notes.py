...
def print_notes(
    ...
    *changes* is a list of strings. It will be sorted and ordered by category
    ...
    print(
        f"The actinia-core {end_tag} release provides more than "
...
def notes_from_gh_api(start_tag, end_tag, branch, categories, exclude):
    ...
    text = subprocess.run(
        [
            "gh",
            "api",
            "repos/OSGeo/actinia-core/releases/generate-notes",
            "-f",
...
def notes_from_git_log(start_tag, end_tag, categories, exclude):
    ...
    text = subprocess.run(
        [
            ...
            "log",
            f"{start_tag}..{end_tag}",
            f"--pretty=format:{PRETTY_TEMPLATE}",
        ],
        capture_output=True,
        text=True,
        check=True,
    ...
    if not commits:
        raise RuntimeError(
            "No commits retrieved from ", "git log (try different tags)"
        )
