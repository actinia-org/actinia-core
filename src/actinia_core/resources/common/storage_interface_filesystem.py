...
class ResourceStorageFilesystem(ResourceStorageBase):
    ...
    def __init__(self, user_id, resource_id, config, resource_url_base=None):
        ...
        self.user_export_path = os.path.join(self.resource_dir,
                                             self.user_id)
        self.resource_export_path = os.path.join(self.user_export_path,
                                                 self.resource_id)
        self.dir_created = False

    def _create_resource_directory(self):
        """Create a new resource directory based on the resource base, the user id and the
        resource id. This directory will contain all exported files of
        the process chain.

        This method will create the directory only once and will ignore repeating calls.
        ...
        if (os.path.isdir(self.resource_dir)
            ...
            if (os.path.isdir(self.user_export_path)
                ...
                if (os.path.isdir(self.resource_export_path)
                    ...
                    raise IOError("Unable to create resource directory <%s>" %
                                  self.resource_export_path)
            else:
                raise IOError("Unable to create resource user directory <%s>" %
                              self.user_export_path)
        else:
            raise IOError("Unable to access the base resource directory <%s>" %
                          self.resource_dir)
    ...
    def store_resource(self, file_path):
        ...
        export_path = os.path.join(self.resource_export_path,
                                   file_name)
