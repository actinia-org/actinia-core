    ...
    'raster': {
        'cost surface': {
            'r.cost': {
                ...
                'description': 'Creates a raster map showing the cumulative cost of '
            ...
            'r.walk': {
                'keywords': 'raster,cost surface,cumulative costs,cost allocation',
                'description': 'Creates a raster map showing the anisotropic cumulative'
                               ' cost of moving between different geographic locations '
                               'on an input raster map whose cell category values '
                               'represent cost.',
                'module': 'r.walk'
            }
        },
        'projection': {
            'r.proj': {
                ...
                'description': 'Re-projects a raster map from given location to '
        ...
        'transect': {
            'r.transect': {
                ...
                'description': 'Outputs raster map layer values lying along user '
                               'defined transect line(s).',
                'module': 'r.transect'
            }
        },
        'series': {
            'r.series.interp': {
                ...
                'description': 'Interpolates raster maps located (temporal or spatial) '
                               'in between input raster maps at specific sampling '
                               'positions.',
                'module': 'r.series.interp'
            },
            'r.series.accumulate': {
                'keywords': 'raster,series,accumulation',
                'description': 'Makes each output cell value a accumulationfunction '
                               'of the values assigned to the corresponding cells in'
        ...
        'terrain': {
            'r.slope.aspect': {
                ...
                'description': 'Generates raster maps of slope, aspect, curvatures '
        ...
        'surface': {
            ...
            'r.resamp.bspline': {
                ...
                'description': 'Performs bilinear or bicubic spline interpolation '
                               'with Tykhonov regularization.',
                'module': 'r.resamp.bspline'
            },
            'r.surf.idw': {
                'keywords': 'raster,surface,interpolation,IDW',
                'description': 'Provides surface interpolation from raster point '
                               'data by Inverse Distance Squared Weighting.',
                'module': 'r.surf.idw'
            },
            'r.surf.gauss': {
                'keywords': 'raster,surface,random',
                'description': 'Generates a raster map using gaussian random number '
                               'generator. Mean and standard deviation of gaussian '
            ...
            'r.contour': {
                ...
                'description': 'Produces a vector map of specified contours from a '
                               'raster map.',
                'module': 'r.contour'
            },
            'r.surf.random': {
                'keywords': 'raster,surface,random',
                'description': 'Produces a raster surface map of uniform random '
        ...
        'sampling': {
            ...
            'r.random': {
                ...
                'description': 'Creates a raster map layer and vector point map '
        ...
        'landscape structure analysis': {
            ...
            'r.li.padcv': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates coefficient of variation of patch area '
                               'on a raster map',
                'module': 'r.li.padcv'
            },
            'r.li.patchnum': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates patch number index on a raster map, using '
                               'a 4 neighbour algorithm.',
                'module': 'r.li.patchnum'
            },
            'r.li.padsd': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates standard deviation of patch area a raster '
                               'map',
                'module': 'r.li.padsd'
            },
            'r.li.cwed': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates contrast weighted edge density index on '
                               'a raster map',
                'module': 'r.li.cwed'
            },
            'r.li.edgedensity': {
            ...
            'r.li.mps': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates mean patch size index on a raster map, '
                               'using a 4 neighbour algorithm',
                'module': 'r.li.mps'
            },
            'r.li.shannon': {
            ...
            'r.li.patchdensity': {
                'keywords': 'raster,landscape structure analysis,patch index',
                'description': 'Calculates patch density index on a raster map, '
                               'using a 4 neighbour algorithm',
        ...
        'tiling': {
            ...
            'r.tileset': {
                'keywords': 'raster,tiling',
                'description': 'Produces tilings of the source projection for use '
                               'in the destination region and projection.',
                'module': 'r.tileset'
            }
        },
        'groundwater flow': {
            'r.gwflow': {
                ...
                'description': 'Numerical calculation program for transient, confined'
        ...
        'reclassification': {
            'r.reclass': {
                ...
                'description': 'Reclassify raster map based on category values. '
                               'Creates a new raster map whose category values are '
        ...
        'category': {
            'r.category': {
                ...
                'description': 'Manages category values and labels associated with '
        ...
        'conversion': {
            ...
            'r.to.rast3elev': {
                ...
                'description': 'Creates a 3D volume map based on 2D elevation and '
                               'value raster maps.',
        ...
        'statistics': {
            ...
            'r.coin': {
                ...
                'description': 'Tabulates the mutual occurrence (coincidence) of '
                               'categories for two raster map layers.',
                'module': 'r.coin'
            },
            'r.support.stats': {
                'keywords': 'raster,statistics',
            ...
            'r.clump': {
                ...
                'description': 'Recategorizes data in a raster map by grouping cells '
                               'that form physically discrete areas into unique '
                               'categories.',
                'module': 'r.clump'
            },
            'r.cross': {
            ...
            'r.reclass.area': {
                ...
                'description': 'Reclasses a raster map greater or less than user '
                               'specified area size (in hectares).',
                'module': 'r.reclass.area'
            },
            'r.univar': {
                'keywords': 'raster,statistics,univariate statistics,zonal statistics',
                'description': 'Calculates univariate statistics from the non-null '
                               'cells of a raster map. Statistics include number of '
            ...
            'r.covar': {
                'keywords': 'raster,statistics',
                'description': 'Outputs a covariance/correlation matrix for user-'
            ...
            'r.mode': {
                ...
                'description': 'Finds the mode of values in a cover map within areas '
            ...
            'r.regression.line': {
                ...
                'description': 'Calculates linear regression from two raster maps: '
                               'y = a + b*x.',
                'module': 'r.regression.line'
            },
            'r.regression.multi': {
                'keywords': 'raster,statistics,regression',
                'description': 'Calculates multiple linear regression from raster '
                               'maps.',
                'module': 'r.regression.multi'
            },
            'r.quant': {
                'keywords': 'raster,statistics,quantization',
                'description': 'Produces the quantization file for a floating-point '
                               'map.',
                'module': 'r.quant'
            },
            'r.stats': {
            ...
            'r.kappa': {
                ...
                'description': 'Calculates error matrix and kappa parameter for '
        ...
        'rescale': {
            'r.rescale.eq': {
                ...
                'description': 'Rescales histogram equalized the range of category '
                               'values in a raster map layer.',
                'module': 'r.rescale.eq'
            },
            'r.rescale': {
                'keywords': 'raster,rescale',
                'description': 'Rescales the range of category values in a raster '
                               'map layer.',
                'module': 'r.rescale'
            }
        },
        ...
        'color table': {
            'r.colors': {
                ...
                'description': 'Creates/modifies the color table associated with '
                               'a raster map.',
                'module': 'r.colors'
            }
        },
        'composite': {
            'r.composite': {
                ...
                'description': 'Combines red, green and blue raster maps into a '
                               'single composite raster map.',
                'module': 'r.composite'
            },
            'r.blend': {
                'keywords': 'raster,composite',
                'description': 'Blends color components of two raster maps by a '
                               'given ratio.',
                'module': 'r.blend'
            }
        },
        ...
        'metadata': {
            'r.describe': {
                ...
                'description': 'Prints terse list of category values found in a '
                               'raster map layer.',
                'module': 'r.describe'
            },
            'r.info': {
            ...
            'r.support': {
                'keywords': 'raster,metadata',
                'description': 'Allows creation and/or modification of raster map '
                               'layer support files.',
                'module': 'r.support'
            },
            'r.region': {
        ...
        'profile': {
            'r.profile': {
                ...
                'description': 'Outputs the raster map layer values lying on user-'
                               'defined line(s).',
                'module': 'r.profile'
            }
        },
        'elevation': {
            'r.fillnulls': {
                ...
                'description': 'Fills no-data areas in raster maps using spline '
                               'interpolation.',
                'module': 'r.fillnulls'
            },
            'r.plane': {
                'keywords': 'raster,elevation',
                'description': 'Creates raster plane map given dip (inclination), '
                               'aspect (azimuth) and one point.',
                'module': 'r.plane'
            },
            'r.relief': {
            ...
            'r.shade': {
                ...
                'description': 'Drapes a color raster over an shaded relief or aspect '
                               'map.',
                'module': 'r.shade'
            }
        },
        'algebra': {
            ...
            'r.neighbors': {
                'keywords': 'raster,algebra,statistics,aggregation,neighbor,focal '
                            'statistics,filter',
                'description': 'Makes each cell category value a function of the '
            ...
            'r.texture': {
                ...
                'description': 'Generate images with textural features from a raster '
                               'map.',
                'module': 'r.texture'
            }
        },
        'fire': {
            'r.spreadpath': {
                ...
                'description': 'Recursively traces the least cost path backwards to '
                               'cells from which the cumulative cost was determined.',
                'module': 'r.spreadpath'
            },
            'r.spread': {
                'keywords': 'raster,fire,spread,hazard,model',
                'description': 'Simulates elliptically anisotropic spread. Generates '
                               'a raster map of the cumulative time of spread, given '
                               'raster maps containing the rates of spread (ROS), the '
            ...
            'r.ros': {
                ...
                'description': 'Generates rate of spread raster maps. Generates three, '
                               'or four raster map layers showing the base '
        ...
        'resample': {
            ...
            'r.resamp.rst': {
                ...
                'description': 'Reinterpolates and optionally computes topographic '
                               'analysis from input raster map to a new raster map '
                               '(possibly with different resolution) using '
            ...
            'r.resamp.stats': {
                'keywords': 'raster,resample',
                'description': 'Resamples raster map layers to a coarser grid using '
                               'aggregation.',
        ...
        'volume': {
            'r.volume': {
                ...
                'description': 'Calculates the volume of data "clumps". Optionally '
                               'produces a GRASS vector points map containing the '
        ...
        'buffer': {
            ...
            'r.buffer.lowmem': {
                'keywords': 'raster,buffer',
                'description': 'Creates a raster map showing buffer zones surrounding '
                               'cells that contain non-NULL category values. This is '
                               'the low-memory alternative to the classic r.buffer '
                               'module.',
        ...
        'solar': {
            'r.horizon': {
                ...
                'description': "Computes horizon angle height from a digital elevation "
                               "model. The module has two different modes of "
                               ...
                               "is the angle ""(in degrees), which is measured "
            ...
            'r.sunmask': {
                ...
                'description': 'Calculates cast shadow areas from sun position and '
                               'elevation raster map. Either exact sun position (A) '
            ...
            'r.sunhours': {
                ...
                'description': "Calculates solar elevation, solar azimuth, and sun "
                               "hours. Solar elevation: the angle between the "
            ...
            'r.sun': {
                ...
                'description': 'Solar irradiance and irradiation model. Computes '
                               'direct (beam), diffuse and reflected solar '
        ...
        'viewshed': {
            'r.viewshed': {
                ...
                'description': 'Computes the viewshed of a point on an elevation '
                               'raster map. Default format: NULL (invisible), '
        ...
        'querying': {
            'r.what': {
                ...
                'description': 'Queries raster maps on their category values and '
                               'category labels.',
                'module': 'r.what'
            },
            'r.what.color': {
        ...
        'distance': {
            'r.grow.distance': {
                ...
                'description': 'Generates a raster map containing distances to nearest '
                               'raster features.',
                'module': 'r.grow.distance'
            },
            'r.distance': {
                'keywords': 'raster,distance',
                'description': 'Locates the closest points between objects in two '
                               'raster maps.',
                'module': 'r.distance'
            },
            'r.grow': {
                'keywords': 'raster,distance,proximity',
                'description': 'Generates a raster map layer with contiguous areas '
                               'grown by one cell.',
                'module': 'r.grow'
            }
        },
        'hydrology': {
            'r.carve': {
                ...
                'description': 'Generates stream channels. Takes vector stream data, '
                               'transforms it to raster and subtracts depth from the '
                               'output DEM.',
                'module': 'r.carve'
            },
            'r.uslek': {
            ...
            'r.flow': {
                'keywords': 'raster,hydrology',
                'description': 'Constructs flowlines. Computes flowlines, flowpath '
                               'lengths, and flowaccumulation (contributing areas) '
            ...
            'r.drain': {
                ...
                'description': 'Traces a flow through an elevation model or cost '
                               'surface on a raster map.',
                'module': 'r.drain'
            },
            'r.sim.sediment': {
                'keywords': 'raster,hydrology,soil,sediment flow,erosion,deposition,'
                            'model',
                'description': 'Sediment transport and erosion/deposition simulation '
            ...
            'r.fill.dir': {
                'keywords': 'raster,hydrology',
                'description': 'Filters and generates a depressionless elevation '
                               'map and a flow direction map from a given elevation '
                               'raster map.',
                'module': 'r.fill.dir'
            },
            'r.water.outlet': {
                'keywords': 'raster,hydrology,watershed',
                'description': 'Creates watershed basins from a drainage direction '
                               'map.',
            ...
            'r.solute.transport': {
                ...
                'description': 'Numerical calculation program for transient, confined '
                               'and unconfined solute transport in two dimensions',
            ...
            'r.sim.water': {
                ...
                'description': 'Overland flow hydrologic simulation using path sampling'
                               ' method (SIMWE).',
                'module': 'r.sim.water'
            },
            'r.topidx': {
                'keywords': 'raster,hydrology,wetness',
                'description': 'Creates a topographic index (wetness index) raster '
                               'map from an elevation raster map.',
                'module': 'r.topidx'
            }
        },
        'geometry': {
            'r.patch': {
                ...
                'description': 'Creates a composite raster map layer by using known '
            ...
            'r.thin': {
                ...
                'description': 'Thins non-null cells that denote linear features '
        ...
        'color transformation': {
            'r.his': {
                ...
                'description': 'Generates red, green and blue (RGB) raster map layers '
                               'combining hue, intensity and saturation (HIS) values '
        ...
        'geomorphology': {
            'r.param.scale': {
                ...
                'description': 'Extracts terrain parameters from a DEM. Uses a multi-'
                               'scale approach by taking fitting quadratic parameters '
    ...
    'general': {
        'support': {
            'g.version': {
                ...
                'description': 'Displays GRASS GIS version info. Optionally also '
                               'prints build or copyright information.',
                'module': 'g.version'
            }
        },
        'settings': {
            'g.mapset': {
                ...
                'description': 'Changes/reports current mapset. Optionally create '
                               'new mapset or list available mapsets in given '
                               'location.',
                'module': 'g.mapset'
            },
            'g.region': {
                'keywords': 'general,settings',
                'description': 'Manages the boundary definitions for the geographic '
                               'region.',
                'module': 'g.region'
            },
            'g.gisenv': {
                'keywords': 'general,settings,variables,scripts',
                'description': "Outputs and modifies the user's current GRASS variable "
                               "settings. Prints all defined GRASS variables if no "
                               "option is given.",
                'module': 'g.gisenv'
            }
        },
        ...
        'map management': {
            'g.list': {
                ...
                'description': 'Lists available GRASS data base files of the user-'
                               'specified data type optionally using the search '
                               'pattern.',
                'module': 'g.list'
            },
            'g.findfile': {
                'keywords': 'general,map management,scripts',
                'description': "Searches for GRASS data base files and sets variables "
                               "for the shell.",
                'module': 'g.findfile'
            },
            'g.remove': {
                'keywords': 'general,map management,remove',
                'description': "Removes data base element files from the user's "
                               "current mapset using the search pattern.",
                'module': 'g.remove'
            },
            'g.rename': {
                'keywords': 'general,map management,rename',
                'description': "Renames data base element files in the user's current "
                               "mapset.",
                'module': 'g.rename'
            }
        }
