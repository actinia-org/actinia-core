...
class GoogleSatelliteBigQueryInterface(object):
    ...
    def query_sentinel2_archive(self, start_time, end_time, lat=None,
                                lon=None, cloud_cover=None, scene_id=None):
        return self._query_satellite_archive(satellite="sentinel2",
                                             start_time=start_time,
                                             end_time=end_time,
                                             lat=lat, lon=lon,
                                             cloud_cover=cloud_cover,
    ...
    def _query_satellite_archive(self, satellite, start_time,
        ...
        Args:
            ...
            lon (float): Longitude coordinates that should intersect the landsat scenes
            ...
            if query_results.result():
                for row in query_results.result():
                    ...
                    result.append(dict(scene_id=scene_id, sensing_time=sensing_time,
                                       ...
                                       cloud_cover=cloud_cover, total_size=total_size))
            ...
            if rows:
                for row in rows:
                    ...
                    for band in bands:
                        ...
                        result[product_id][band]["public_url"] = public_url + tile_name
                        result[product_id][band]["gcs_url"] = gcs_url + tile_name
