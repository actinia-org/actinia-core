...
"""
from actinia_core.core.google_satellite_bigquery_interface import GoogleSatelliteBigQueryInterface
...
class GoogleBigQueryInterfaceTestCase(unittest.TestCase):
    ...
    def test_query_for_landsat_scene_time_coords(self):
        ...
        result = gsqi.query_landsat_archive(start_time=start, end_time=end, lon=-45.0, lat=-7.0)
        pprint(result)
        self.assertTrue(len(result) >= 1)
    ...
    def test_query_for_sentinel2_scene_time_coords(self):
        ...
        result = gsqi.query_sentinel2_archive(start_time=start, end_time=end, lon=-45.0, lat=-7.0)
        pprint(result)
        self.assertTrue(len(result) >= 1)
